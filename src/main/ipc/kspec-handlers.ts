...
import { type AIBackend, getInstructionFilePath, readInstructionFile, writeInstructionFile, ensureAiderConfig } from './instruction-files'
...
export function installTaskInstructions(projectPath: string, backend: 'beads' | 'kspec', aiBackend: AIBackend = 'claude'): boolean {
  try {
...
export function registerKspecHandlers() {
  ...
  // Delete a task via CLI (daemon doesn't have a DELETE endpoint)
  ipcMain.handle('kspec:delete-task', async (_event, cwd: string, taskRef: string) => {
    try {
      const ref = taskRef.startsWith('@') ? taskRef : `@${taskRef}`
      const result = await spawnCommand('kspec', ['task', 'delete', '--refs', ref, '--force'], cwd)
      console.log('[kspec:delete-task]', { cwd, ref, code: result.code, stdout: result.stdout, stderr: result.stderr })
      if (result.code !== 0) {
        const details = [
          result.stderr?.trim(),
          result.stdout?.trim(),
        ...
        return { success: false, error: details || 'Failed to delete task (no output)' }
      }
      return { success: true }
    } catch (e) {
      console.error('[kspec:delete-task] exception:', e)
      return { success: false, error: `Spawn failed: ${String(e)}` }
    }
  })
}
