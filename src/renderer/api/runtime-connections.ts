...
export async function rememberRuntimeCredential(credentialRef: string, token: string, persist = true): Promise<void> {
  credentials.set(credentialRef, token)
  if (persist) await storeDeviceCredential(credentialRef, token)
}
...
export async function attachRuntimeConnection(
  ...
  if (token !== undefined) await rememberRuntimeCredential(saved.credentialRef, token, persistCredential)
